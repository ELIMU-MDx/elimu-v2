<tr class="{{$even ? 'bg-gray-100' : 'bg-white'}} relative z-0">
    ...
    @foreach($this->results->sortBy('target') as $result)
        <td class="px-3 py-4 whitespace-nowrap text-sm text-center border-l border-gray-300">
            {{$result->averageCQ->toString() ?: 'NaN'}}

            @if($result->averageCQ->rounded())
                <div class="text-xs text-gray-500 mt-2">{{$result->measurements->included()->standardDeviationCq()->toString()}}</div>
            @endif
        </td>
        ...
        <td class="px-3 py-4 whitespace-nowrap text-sm text-center"
            x-data="{show: false}">
            <p>{{$result->measurements->included()->count()}}
                / {{$result->measurements->count()}}</p>
            ...
            <div>
                <ul class="flex -mx-2 divide-x divide-gray-200 grid-cols-4 justify-center items-center"
                    x-show="show" x-cloak>
                    @foreach($result->measurements as $key => $measurement)
                        <li class="px-2">
                            <button wire:click="toggleExcluded('{{$result->target}}', {{$key}})"
